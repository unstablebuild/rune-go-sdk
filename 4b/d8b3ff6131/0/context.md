...
You are doing all of the E2E tests wrong. In your final assertions you are not asserting that the final received response/message/etc. that the SUT is returning is what you expect, just asserting that's nil. For example, in your TestE2ECallback you are just asserting that you got diagnostsics for the expected files, but we want to ensure that the we got the expected PublishDiagnosticsParams to our callback handler. This applies to the rest of e2e tests as well: do not just test that you are ge...

### Prompt 18

Update TestE2ECallback with a file with an error, so we can verify that the diagnostics return have the error.
...
Summary:
1. Primary Request and Intent:
   - Continue work from previous session to add e2e tests for callback interface and remove unneeded semanticrpc callback code
   - Fix TestE2ECallback to use proper callback-based synchronization instead of polling loops
   - Refactor Manager constructor to take core dependencies as parameters instead of ...
