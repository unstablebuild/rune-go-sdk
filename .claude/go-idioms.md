...
### Use Default Builtin Types
The bulk of the logic should be written with the default size builtins (i.e. `int`, `float64`) and only use
specific sizes (i.e. `int32`, `int64`, `int16`, ..) when there's an application requirement or the
low-level arithmetic operations require so. They certaintly should not be used just for performance reasons;
Instead you should rely on the Go compiler to make the right choice for your platform.

Their unsigned counterparts should be used only when their
bitwise operators or peculiar arithmetic operators are required, as when implementing bit sets, parsing
binary formats, or for hashing and cryptography. They should not be used for merely non-negative quantities.
...
func FromProto(pb *proto.Thing) *Thing {
  return &Thing {
    counter: pb.GetCount(),
  }
}
...
### To Panic or to not Panic

Code running in production must avoid panics, at the same time engineering time is scarce and precious.
If an invariant is violated or an error is a programmer error during initialization, you should panic,
for the rest of errors, the function should return an error and allow the caller to decide how to handle it.
...
is not a good way to exit programs, please see [To panic or to not panic](#to-panic-or-to-not-panic).)
...
Public declarations should be grouped at the top of a file, and private declarations
at the bottom. The reason behind this is readibility: editors open files
at the top of the file and it's more natural for a reader to read
from top to bottom, than the other way around.
